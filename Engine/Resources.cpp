...
namespace Engine {
    ...
    std::shared_ptr<ALLEGRO_BITMAP> Resources::GetBitmap(std::string name) {
        ...
        ALLEGRO_BITMAP* raw = al_load_bitmap(bitmapPath.c_str());
        if (!raw) {
            static std::shared_ptr<ALLEGRO_BITMAP> placeholder;   // created once
            if (!placeholder) {
                placeholder.reset(al_create_bitmap(1, 1), al_destroy_bitmap);
                // make it fully transparent
                ALLEGRO_BITMAP* old = al_get_target_bitmap();
                al_set_target_bitmap(placeholder.get());
                al_clear_to_color(al_map_rgba(0, 0, 0, 0));
                al_set_target_bitmap(old);
            }
            LOG(WARN) << "Missing bitmap: " << bitmapPath
                    << " – using 1×1 transparent placeholder.";
            bitmaps[name] = placeholder;          // cache the stub as well
            return placeholder;
        }   
        LOG(INFO) << "Loaded Resource<image>: " << bitmapPath;
        bitmaps[name] = std::shared_ptr<ALLEGRO_BITMAP>(raw, al_destroy_bitmap);
        return bitmaps[name];
    }
